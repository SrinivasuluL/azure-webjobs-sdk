...
namespace Microsoft.Azure.WebJobs.Host.Executors
{
    ...
    internal class FunctionExecutor : IFunctionExecutor
    {
            ...
            ParameterHelper parameterHelper, TraceLevel functionTraceLevel, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using (functionCancellationTokenSource)
                {
                    ...
                    using (parameterHelper.GetDisposables())
                    {
                        if (functionTraceLevel >= TraceLevel.Info)
                        {
        ...
        internal class ParameterHelper
        {
            ...
            private object[] _invokeParameters;
            ...
            public object[] InvokeParameters => _invokeParameters;

            internal static ParameterHelper New(object[] args)
            {
                return new ParameterHelper(null) { _invokeParameters = args };
            }
            ...
            // Get a single IDisposable that will call IDisposable on any of the IValueProviders in our parameter list. 
            public IDisposable GetDisposables()
            {
                return ValueProviderDisposable.Create(this._parameters);
            }
            ...
            public async Task<IDelayedException> PrepareParametersAsync()
            {
                ...
                this._invokeParameters = reflectionParameters;
            ...
            public async Task ProcessOutputParameters(CancellationToken cancellationToken)
            {
                ...
                foreach (string name in parameterNamesInBindOrder)
                {
                    ...
                    if (binder != null)
                    {
                        object argument = _invokeParameters[this.GetParameterIndex(name)];
